...
# -*- coding: utf-8 -*-
"""
scripts/build_5m_patterns_kb_v2.py

Build 5m direction-sequence pattern KB from mined parquet.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        default="data/btcusdt_5m_patterns_L2_11.parquet",
        help="Input parquet with mined patterns.",
    )
    parser.add_argument("--kb", default="kb/btcusdt_5m_knowledge.yaml", help="KB file to update.")
...
def strength_bucket(acc: float, lift: float, support: int) -> str:
    if support >= 400 and acc >= 0.65 and lift >= 0.08:
        return "very_strong"
    if support >= 250 and acc >= 0.60 and lift >= 0.05:
        return "strong"
    if support >= 120 and acc >= 0.55:
        return "medium"
    if acc >= 0.52:
        return "weak"
    return "very_weak"
...
def ensure_patterns_container(kb: Dict[str, Any]) -> None:
    ...
    kb["patterns"].setdefault("dir_sequence_5m", {})
    kb["patterns"]["dir_sequence_5m"].setdefault("version", "v1")
...
def main() -> None:
    ...
    for _, row in df.iterrows():
        ...
        items.append(
            {
                "id": f"PAT5M_DIR_L{length}_{len(items)+1:03d}",
                "timeframe": "5m",
                "pattern_type": "dir_sequence_forward",
                "source": {
                    "dataset": "btcusdt_5m",
                    "miner": "mine_5m_dir_sequences_v2",
                    "discovered_at": now_iso,
                    "discovered_from": "5m_only",
                },
                "sequence": {
                    "dirs": dirs,
                    "length": length,
