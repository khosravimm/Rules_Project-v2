...
# -*- coding: utf-8 -*-
"""
scripts/inspect_4h_micro_patterns_v2.py

Inspect micro-structure patterns (5m inside 4h) from KB.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inspect 4h micro-patterns (from 5m).")
    parser.add_argument("--kb", "-k", default="kb/btcusdt_4h_knowledge.yaml", help="Path to 4h KB.")
    parser.add_argument(
        "--bucket",
    ...
    parser.add_argument("--context-length", type=int, default=None, help="Filter by context length.")
...
def load_patterns(kb_path: Path) -> List[MicroPattern]:
    if not kb_path.exists():
        print(f"[ERROR] KB not found: {kb_path}")
        raise SystemExit(1)
    kb = yaml.safe_load(kb_path.read_text(encoding="utf-8")) or {}
    items = kb.get("patterns", {}).get("intra_4h_from_5m", {}).get("items", [])
    ...
    for item in items:
        ...
        patterns.append(
            MicroPattern(
                id=str(item.get("id", "")),
                context_length=int(context.get("length") or 0),
                ...
                micro_features={k: v for k, v in (micro_pat.get("features", {}) or {}).items()},
            )
        )
    return patterns


def filter_patterns(patterns: List[MicroPattern], args: argparse.Namespace) -> List[MicroPattern]:
    out: List[MicroPattern] = []
    for p in patterns:
        if args.bucket != "all" and p.strength_bucket != args.bucket:
            continue
        if args.status != "all" and p.status != args.status:
            continue
        if args.context_length is not None and p.context_length != args.context_length:
            continue
...
def sort_patterns(patterns: List[MicroPattern], key: str, descending: bool) -> List[MicroPattern]:
    if key == "strength":
...
def format_table(patterns: List[MicroPattern]) -> str:
    headers = ["id", "ctx", "fav", "str", "sup", "acc", "base", "lift", "status"]
    rows = []
    for p in patterns:
        rows.append(
            [
...
def main() -> None:
    args = parse_args()
    patterns = load_patterns(Path(args.kb))
    total = len(patterns)
                ...
                [
                    {
                        ...
                        "status": p.status,
                        "tags": p.tags,
                        "micro_features": p.micro_features,
                    }
                    for p in patterns
                ],
                indent=2,
