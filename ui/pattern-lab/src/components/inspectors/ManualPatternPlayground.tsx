...
const ManualPatternPlayground = () => {
  ...
  const Summary = () => {
    ...
    return (
      <div className="glass-panel p-3 mt-3">
        <p className="text-slate-200 text-sm">Occurrences: {s.approx_support}</p>
        <p className="text-slate-200 text-sm">Winrate: {s.approx_winrate ? (s.approx_winrate * 100).toFixed(1) + "%" : "-"}</p>
        <p className="text-slate-200 text-sm">Direction hint: {s.direction_hint}</p>
      </div>
    );
  };
    ...
    return (
      <div className="mt-3 max-h-48 overflow-y-auto space-y-1">
        {candidateResult.occurrences.map((o: CandidateOccurrence) => (
          <div key={o.start_ts} className="flex items-center justify-between bg-white/5 rounded-lg px-2 py-1">
  ...
  return (
    <div className="glass-panel p-4">
      ...
      {!candidateWindow ? (
        <p className="text-slate-300 text-sm">Enable “Select window” and click two candles to define a range.</p>
      ) : (
        <div className="space-y-2 text-slate-200 text-sm">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={savePattern} disabled={saving} className="button-ghost border-emerald-400 text-emerald-200">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <textarea
              className="bg-white/5 border border-white/10 rounded-lg px-2 py-1 col-span-2"
              rows={3}
